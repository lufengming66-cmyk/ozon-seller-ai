...
from license import get_hardware_id, verify_license_key
import os
...
# ==================== 设备授权拦截器 ====================
# 检查授权状态
license_status = load_config('license_status', 'unactivated')
...
if license_status != 'activated':
    ...
    <style>
        ...
        .stButton>button {
            ...
            font-weight: 600;
            font-size: 1.1rem;
            background: linear-gradient(90deg, #005BFF 0%, #2979ff 100%);
            border: none;
            color: white;
            transition: all 0.3s ease;
        }
    ...
    # 水平居中布局 - 三列布局（左右留白，中间放卡片）
    col1, col2, col3 = st.columns([1, 1.5, 1])
    
    with col2:
        # 卡片内容
        st.title("🔐 Ozon Seller Pro")
        st.caption("专业跨境电商运营工作站 · 商业版")
        st.divider()
        ...
        if os.path.exists(qr_path):
            st.image(qr_path, use_column_width=False, width=200)
        
        # 引流文案
        st.success("🎉 欢迎使用 Ozon Seller Pro 智能工作站！")
        st.warning("🔒 本软件实行「一机一码」安全绑定。请复制下方您的专属机器码，添加作者微信：sensen1017，并发送暗号『Ozon提效』获取永久授权！")
        
        # 显示机器码
        st.code(hwid, language=None)
        
        st.info("👉 扫码/加微信 **你的微信号** 回复【机器码】获取授权密钥")
        ...
        # 激活按钮
        if st.button("🚀 激活软件", type="primary", use_container_width=True):
            if verify_license_key(input_key, hwid):
                # 激活成功
                save_config('license_status', 'activated')
                st.balloons()
                st.success("🎉 激活成功！欢迎使用 Ozon Seller Pro")
                st.rerun()
            else:
                st.error("❌ 授权密钥无效，请检查后重试")
